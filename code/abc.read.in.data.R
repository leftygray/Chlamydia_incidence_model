...
m.population <- do.call(cbind,population.data[,1:101])
f.population <- do.call(cbind,population.data[,102:202])
...
m.14 <- c(m.14,m.14[12]); m.19 <- c(m.19,m.19[12]); m.24 <- c(m.24,m.24[12]); m.29 <- c(m.29,m.29[12]); m.34 <- c(m.34,m.34[12]); f.14 <- c(f.14,f.14[12]); f.19 <- c(f.19,f.19[12]); f.24 <- c(f.24,f.24[12]); f.29 <- c(f.29,f.29[12]); f.34 <- c(f.34,f.34[12])
...
for (i in 1:4) {for (j in 1:12) {tested.f[i,j] <- as.numeric(as.character(nndss.data[j+3,i+19]))}}
